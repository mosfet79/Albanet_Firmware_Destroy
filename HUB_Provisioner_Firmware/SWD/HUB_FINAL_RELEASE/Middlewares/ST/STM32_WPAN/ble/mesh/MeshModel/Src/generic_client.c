...

{
  ...
  */
  ...
  TRACE_M(TF_MISC,"EXECUTION FROM GENERIC_CLIENT!!!\r\n");
...

{
  ...
  */
  ...
  TRACE_M(TF_MISC,"DECODING BLE MESSAGE STRUCTURE---> \r\n");
#endif
...
#if(ALBANET_DEBUG_STATE == 1)
  TRACE_M(TF_MISC,"The Message Buffer: %d !!!\r\n", *msg_buff);
#endif
...
#if(ALBANET_DEBUG_STATE == 1)
  TRACE_M(TF_MISC,"MODEL ID: %.02x \r\n", GENERIC_MODEL_CLIENT_ONOFF_MODEL_ID);
  TRACE_M(TF_MISC,"Element Number: %.02x \r\n", element_number);
  TRACE_M(TF_MISC,"MSG OpCode: %.02x \r\n", msg_opcode);
  TRACE_M(TF_MISC,"Ack_Flag: %02x \r\n", ack_flag);
#endif
