...
MOBLE_RESULT Albanet_Light(uint8_t state, uint8_t group)
{

#if(ALBANET_DEBUG_STATE == 1)
  TRACE_M(TF_MISC,"FUNCTION EXECUTION HUB ALBANET ...!\r\n");
#endif

  AppliOnOffSet.Present_OnOff = state;
  AppliOnOffSet.Present_OnOffValue = 32,000;

  TRACE_M(TF_MISC,"CONDITION STATE VALUE ... #### ... %d !\r\n", state);


  if (state == 1){
#if(ALBANET_DEBUG_STATE == 1)
	  TRACE_M(TF_MISC,"SENDING INSTRUCTION LIGHT_ON FOR BOT ...!\r\n");
#endif

	  Appli_LightPwmValue.IntensityValue = 300;

	  AppliOnOffSet.TargetValue = APPLI_LED_ON;
...
#if(ALBANET_DEBUG_STATE == 1)
	  TRACE_M(TF_MISC,"TURN ON LIGHT IN BOT %d ...!\r\n", group);
      TRACE_M(TF_SERIAL_CTRL,"Instruction for #8202: %02hx!\n\r",AppliOnOffSet.Present_OnOff);
#endif
	  ...
	  AppliOnOffSet.Present_OnOff == APPLI_LED_OFF;
      Appli_LightPwmValue.IntensityValue = 32000;
#if(ALBANET_DEBUG_STATE == 1)
      TRACE_M(TF_MISC,"TURNING OFF LIGHT IN BOT %d ...!\r\n", group);
#endif
      Light_UpdateLedValue(LOAD_STATE , Appli_LightPwmValue);
                                     ...
                                     MOBLEUINT8 OptionalValid)
{
  ...
  TRACE_M(TF_MISC,"VARIABLE INSERTED, FROM STANDARD %x02 ..!\r\n", OptionalValid);
  ...
  {
   TRACE_M(TF_MISC,"COMPARING TRANSITION!!!\r\n");
    ...
    if(AppliOnOffSet.Present_OnOffValue > 8000)
    {
      BSP_LED_On(LED_BLUE);
    }
    else
  ...
  {
    TRACE_M(TF_MISC,"ONLY EXECUTING VALUES...\r\n");

    if((AppliOnOffSet.Present_OnOff == APPLI_LED_ON) && (OptionalValid == NO_TRANSITION))
	  {
      TRACE_M(TF_MISC,"SECOND SCENARIO ON EXECUTION...\r\n");
      Appli_LightPwmValue.IntensityValue = PWM_TIME_PERIOD;

      //Appli_GenericClient_OnOff_Albanet_Set(1);
      //Appli_GenericClient_Level_Set_Unack();
      //Appli_LightClient_Lightness_Set();

      AppliOnOffSet.TargetValue = APPLI_LED_ON;
      /*This action update the value on Power State*/
      Light_UpdateLedValue(LOAD_STATE , Appli_LightPwmValue);
      TRACE_M(TF_MISC,"TURN ON MAIN LIGHT, ALBANET ...!\r\n");
      /*This only a local effect on HUB*/
      BSP_LED_On(LED_BLUE);
    }
    else
    {
      TRACE_M(TF_MISC,"THIRD SCENARIO ON EXECUTION...\r\n");
      Appli_LightPwmValue.IntensityValue = PWM_VALUE_OFF;
      AppliOnOffSet.TargetValue = APPLI_LED_ON;
      /*This action update the value on Power State*/
      //Appli_GenericClient_OnOff_Albanet_Set(0);


      Light_UpdateLedValue(RESET_STATE , Appli_LightPwmValue);
      TRACE_M(TF_MISC,"TURN OFF MAIN LIGHT, ALBANET ..!\r\n");
      /*This only a local effect on HUB*/
      BSP_LED_Off(LED_BLUE);
...
MOBLE_RESULT Albanet_Light_Dimmer(uint16_t dimmer_value)
{
#if(ALBANET_DEBUG_DIMMER == 1)
	TRACE_M(TF_MISC, "Executing Function ALBANET LIGHT DIMMER \r\n");
#endif

	Appli_LightPwmValue.IntensityValue = dimmer_value;
#if(ALBANET_DEBUG_DIMMER == 1)
	TRACE_M(TF_MISC, "Catching Value about Intensity (Hex Value) %.02x \r\n", Appli_LightPwmValue);
#endif
...
MOBLE_RESULT Appli_Generic_OnOff_Status(MOBLEUINT8 const *pOnOff_status, MOBLEUINT32 plength)
{
  ...
  TRACE_M(TF_MISC,"APPLI GENERIC ONOFF STATUS ...!\r\n");
