...
MOBLE_RESULT Appli_BleSetTxPowerCb(void)
{
  ...
  */ 
  ...
  status = aci_hal_set_tx_power_level(1, 7);

  TRACE_M(TF_MISC,"Adjusting TX Power Output!!\r\n", status);
...
{ 
  ProvisionFlag = 1;
  	...
  	/* THESE ELEMENTS ARE FOR TESTING CONSIDERING MAC ADDRESS OF */

//	uint8_t srcElementAddr = BLEMesh_GetAddress();
//	MOBLEUINT16 dataBufLen = 0;
//	MOBLE_ADDRESS dstAddr;
//	MOBLE_ADDRESS data[6];
//
//	data[0] = 0x20;
//	data[1] = 0x20 >> 8;
//	dataBufLen = 2;
//
//	uint8_t Trasfer_Data = 0xA2;
//
//	uint8_t ALBANET_DATA[] = {0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6};
//
//	/*LINK TO SEND FREQUENTLY TO THE SYSTEM I2c*/
//	Extract_x29();
//
//	  TRACE_I(TF_PROVISION,"ALBANET (appli_mesh.c [1011])Starting Provisioner Mode \r\n");
//
//	  uint8_t albanet_frames[22]= {};
//	  if (HAL_I2C_Mem_Read(&hi2c1, ESP32_ADDR, 0x0000, 1, albanet_frames, sizeof(albanet_frames), 5) != 1){
...
//	  }
  ...
  /*Action to Read I2C and store using BUFFER EXCLUSIVE FOR ESP32*/
  /* HAL_I2C_MEM_WRITE(A, B, C, D, E, F, G)
   * A = HandleTypeDef
   * B = Device Address
   * C = Memory Section Address
   * D = Memory Section Size
   * E = Data to Include
   * F = Data to Include Size
   * G = Timeout of Process
   *
   * */

	//if (HAL_I2C_Mem_Write(&hi2c1, ESP32_ADDR, 0x0000, 1, ALBANET_DATA, sizeof(ALBANET_DATA), 5) != 1){
	//  	BSP_LED_Toggle(LED_RED);
	//}
...
//	uint8_t data_extracted[22] = {};
	...
	/*Action to obtain MACADDRESS and compare the element */
  	/*
  	 * This is the process to read and catch from memory the information
  	 *                         HAL_I2C_MEM_READ(A, B, C, D, E)
  	 * A = Handle TypeDef
  	 * B = Device Address
  	 * C = Address for extract data
  	 * D = Length space to locate data
  	 * E = Variable to Store Data
  	 * F = Space Reservation on Memory to Store Data
  	 * G = TimeOut
  	 * */

	//if (HAL_I2C_Mem_Read(&hi2c1, ESP32_ADDR, 0x0000, 1, data_extracted, sizeof(data_extracted), 5) != 1){
...
/*The action will be similar to the system that calls the push button*/
static void AppliMeshSW1Task(void)
{
  BSP_LED_On(LED_RED);
  TRACE_M(TF_MISC,"PRESSING ALBANET HUB SW1...!\r\n");
  Appli_UpdateButtonState(BSP_PB_GetState(BUTTON_SW1) == BUTTON_PRESSED);
  BSP_LED_Off(LED_RED);
  return;
}

/*This is the function to integrate X29 messages from cloud*/
static void Extract_x29(void){
	...
	/*Allocate the information considering the elements individually*/

	/*Integrate the message using LIGHT ON and LIGHT OFF considering values of specific bytes*/

	/*Modify the PWM integrated for specific BOT */
