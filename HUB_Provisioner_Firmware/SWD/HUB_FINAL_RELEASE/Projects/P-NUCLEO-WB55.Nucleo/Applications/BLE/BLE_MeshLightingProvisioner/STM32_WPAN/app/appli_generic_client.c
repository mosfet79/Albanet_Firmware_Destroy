...
MOBLE_RESULT Appli_GenericClient_OnOff_Albanet_Set(uint8_t);

/* Private functions ---------------------------------------------------------*/

MOBLE_RESULT Appli_GenericClient_OnOff_Albanet_Set(uint8_t value)
{
  /*Element Address*/
  ...
  TRACE_M(TF_MISC,"Element Address: %.02x !!!\r\n",elementAddr);
#endif

  if (value == 0){
	  Led_Value=APPLI_LED_OFF;
  }
  if(value == 1){
...
{ 
  MOBLE_ADDRESS elementAddr = 0;
  MOBLEUINT8 pGeneric_OnOffParam[2];
  TRACE_I(TF_PROVISION,"ALBANET (appli_generic.c [56])Executing ON/SET \r\n");
  //Led_Value ^= APPLI_LED_ON;
  Led_Value = APPLI_LED_ON;
  pGeneric_OnOffParam[0] = Led_Value;
  //pGeneric_OnOffParam[0] = APPLI_LED_OFF;
  GenericClient_OnOff_Set_Unack(elementAddr,
                          (_Generic_OnOffParam*) pGeneric_OnOffParam, 
                          sizeof(pGeneric_OnOffParam) );
