...
{
TRACE_M(TF_MISC,"BLE MESH MODELS COMMAND\r\n");
                                              ...
                                              MOBLEUINT8 isVendor)
{
    ...
    TRACE_M(TF_MISC,"Getting BLE Mesh Address <--- : %.02x \r\n", srcAddress);
#endif
...
#if(ALBANET_DEBUG_STATE == 1)
    TRACE_M(TF_MISC,"Getting address to send ---> : %.02x \r\n", srcAddress);
    TRACE_M(TF_MISC,"MODEL ID: %.02x\r\n", GENERIC_MODEL_CLIENT_ONOFF_MODEL_ID);
    TRACE_M(TF_MISC,"SourceAddress: %.02x \r\n", srcAddress);
